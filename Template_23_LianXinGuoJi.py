...
class Template_LianXinGuoJi(ParsingTemplate):
    ...
    def _prepare_returndata_regex(self):
        ...
        def _yellowclear_approver(value: str):
            # YellowClear > Yellow Clear
            if value.lower().strip().replace(' ', '') == 'yellowclear':
                return 'Yellow Clear'
            # YellowClear > Light Yellow Clear
            elif value.lower().strip().replace(' ', '') == 'lightyellow':
                return 'Light Yellow Clear'
            return value
        ...
        # def _combine_value_and_bracket(value:str, all_values:List[Tuple[str, AnchorSpec]]):
        #     for item, anchor in all_values:
        ...
        self.retData.transparency.update_kwargs(regex='Appearance|外觀')
    ...
    def _parse_update_category(self, target_ocr_data: dict):
        # 定位 '檢驗結果' 的標題位置
        value_title_label, value_title_anchor = self.get_keyword(target_ocr_data, *self.entire_box, '檢驗結果')
        self.syslogger.info(f'value_title_label: {value_title_label}, value_title_anchor: {value_title_anchor}')
       
       # 擴大範圍
        value_title_anchor.move_x(-value_title_anchor.wid*0.25)
        value_box = (value_title_anchor.btm, self.hgt, value_title_anchor.lft, value_title_anchor.rgt)
        
        # 提取範圍內的所有值
        values = self.get_keywords(target_ocr_data, *value_box, '.+')

        # 使用 `group_ocr_anchor_list` 進行分組
        grouped_values = self.group_ocr_anchor_list(values)
        self.syslogger.info(f"Grouped values: {grouped_values}")
        
        # # 合併每組的OCR box
        # merge_values = [self.merge_boxes(group) for group in grouped_values]
        # self.syslogger.info(f"merge_values: {merge_values}")
        ...
        for key, keyinfo in self.retData.get_all_api_key_and_info():
            ...
            # 查找檢驗項目名稱的標框
            item_label, item_label_anchor = self.get_keyword(target_ocr_data, *item_box, keyinfo.regex)
            self.syslogger.info(f'item_label: {keyinfo}, item_label_anchor: {item_label_anchor}')
            
            if item_label is not None:
                keyinfo.default_enable=True

                # 最近值匹配
                value_label, value_anchor = self.map_closest_value_of_title_by_y_axis(item_label_anchor, merged_values)
                self.syslogger.info('value_label: {value_label}, value_anchor: {value_anchor}')
    ...
    def _parse_birth(self, target_ocr_data, *box) -> Optional[datetime]:
        ...
        self.syslogger.debug(f"matched_str: {matched_str}, matched_anchor: {matched_anchor}")
        if matched_str is not None:
    ...
    def _parse_gender(self, target_ocr_data, *box) -> str:
        ...
        self.syslogger.debug(f"_parse_gender: matched_str: {matched_str}, matched_anchor: {matched_anchor}")
        if matched_str is not None:
    ...
    def _parse_exam_date(self, target_ocr_data, *box) -> str:
        ...
        self.syslogger.debug(f"_parse_exam_date: matched_str: {_matched_str}, matched_anchor: {_matched_anchor}")
