def _parse_update_category(self, target_ocr_data: dict):
    ...
    for i, (text, anchor) in enumerate(raw_values):
        if temp_text:
            # 判斷是否應合併：括號開頭，或 x 軸相鄰，或 y 軸靠近且 x 軸重疊
            if text.startswith("(") or (
                abs(temp_anchor.ymid - anchor.ymid) < temp_anchor.hgt * 1.5 and
                abs(temp_anchor.rgt - anchor.lft) < temp_anchor.wid * 0.5
            ) or (
                abs(temp_anchor.btm - anchor.top) < temp_anchor.hgt * 0.5 and
                (temp_anchor.lft <= anchor.rgt and temp_anchor.rgt >= anchor.lft)
            ):
                temp_text += f" {text}"
